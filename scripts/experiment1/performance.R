...
performanceN <- data.frame( names = c('2', '3'),
...
ggplot(performanceN)+
  ...
  geom_errorbar(aes(x=names, ymin = c(perf_n2$ymin, perf_n3$ymin), ymax = c(perf_n2$ymax, perf_n3$ymax)), width = 0.4, col = 'orange', size = 1.3)+
...
resta_perf <- function(data, area2prop){
  ...
  # and returns the subtraction between performance with 3 alternatives and performance with 2 
...
ggplot(resta_perf_sujetos, aes(x=names, y = values))+
  ...
  ggtitle('Performance (3 alternatives - 2 alternatives)')+
  xlab('Task difficulty (stimulus2/stimulus1)')+
  ylab('Performance (3 - 2 alternatives)')+
  ylim(c(-0.1,0.1))